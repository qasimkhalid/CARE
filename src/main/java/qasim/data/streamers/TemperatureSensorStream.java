...
public class TemperatureSensorStream extends RdfStream implements Runnable {
    ...
    public TemperatureSensorStream( String iri, int sleepTime, String DataStreamFile ) {
        super(iri);
        this.iri_events = iri;
    ...
    public void run() {
        synchronized (DataStreamFile) {
            try {
//                System.out.println("Getting a data from Temprature Stream");
            ...
            try {
                ...
                while ((strLine = reader.readLine()) != null && !keepRunning) {
                    ...
                    if ((!time_check.equals(time_check_old)) && (reader.getLineNumber() > 1)) {
                        try {
//                            System.out.print("Temperature Observer Sleeps for " + observer_sleep + " at Time " + time_check_old + "\n");
